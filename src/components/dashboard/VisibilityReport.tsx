...
import {convertToCSV} from '@/lib/mockData';
import {transformVisibilityData} from '@/lib/api/transform';
import type {
  CompanyData,
  VisibilityReport as VisibilityReportType,
} from '@/lib/types';
...
export function VisibilityReport({
  ...
  useEffect(() => {
    ...
    if (visibilityData) {
      const transformedReport = transformVisibilityData(
        visibilityData,
        selectedModels
      );
      setReport(transformedReport);
    }
  }, [visibilityData, selectedModels]);
  ...
  const handleDownloadCSV = () => {
    ...
    link.setAttribute(
      'download',
      `visibility-report-${report.companyName
        .toLowerCase()
        .replace(/\s+/g, '-')}.csv`
    );
  ...
  const queriesByCategory =
    report?.queryLog.reduce((acc, query) => {
      ...
      <div className='max-w-7xl mx-auto space-y-6'>
        ...
        <div className='space-y-2'>
          ...
          <p className='text-muted-foreground'>
            Here's an overview of your brand's visibility across AI models.
          </p>
        </div>
          ...
          <Card className='bg-card border-border row-span-2'>
            <CardContent className='p-6 flex flex-col items-center justify-center h-full'>
              ...
              <div className='relative w-32 h-32'>
                <svg className='transform -rotate-90 w-32 h-32'>
                  ...
                  <circle
                    ...
                    strokeDashoffset={`${
                      2 * Math.PI * 56 * (1 - report.overallScore / 100)
                    }`}
              ...
              <p className='text-xs text-muted-foreground/70 mt-4 text-center'>
                Your brand's overall visibility in AI responses
              </p>
            </CardContent>
          </Card>
          ...
          <Card className='bg-card border-border'>
            <CardHeader>
              ...
              <p className='text-sm text-muted-foreground'>
                How often your brand appears in different query types.
              </p>
            </CardHeader>
              ...
              {report.competitors.map((competitor, idx) => (
                <div
                  ...
                  <span className='text-primary font-bold'>
                    {Math.round(competitor.visibility)}%
                  </span>
                </div>
              ))}
            </CardContent>
            ...
            <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4'>
              <div>
                ...
                <p className='text-sm text-muted-foreground mt-1'>
                  Queries grouped by category - click to expand
                </p>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            ...
            <div className='space-y-1'>
              {Object.entries(queriesByCategory).map(([category, queries]) => {
                ...
                const mentionedCount = categoryQueries.filter(
                  (q) => q.mentioned
                ).length;
                const visibilityPercent = Math.round(
                  (mentionedCount / categoryQueries.length) * 100
                );
                    ...
                    <button
                      ...
                      <div className='flex items-center gap-4 flex-1'>
                        ...
                        <div className='text-left'>
                          ...
                          <p className='text-sm text-muted-foreground'>
                            {categoryQueries.length} prompts
                          </p>
                        </div>
                      </div>
                      ...
                      <div className='flex items-center gap-6'>
                        ...
                        <div className='flex items-center gap-3 min-w-[200px]'>
                          ...
                          <span className='text-foreground font-semibold min-w-[45px]'>
                            {visibilityPercent}%
                          </span>
                        </div>
              ...
              <div className='space-y-3'>
                <div className='flex gap-3'>
                  ...
                  <div>
                    ...
                    <p className='text-sm text-muted-foreground'>
                      Your brand is rarely mentioned in comparison queries.
                      Focus on content that directly compares your features
                      against competitors.
                    </p>
                  </div>
                </div>
                <div className='flex gap-3'>
                  ...
                  <div>
                    ...
                    <p className='text-sm text-muted-foreground'>
                      Your reviews visibility is low. Encourage satisfied
                      customers to leave reviews on popular platforms to improve
                      mentions.
                    </p>
                  </div>
                </div>
